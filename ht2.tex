...
Im Rahmen dieser Arbeit wurde ein kleines Serius Game implementiert das in VR gespielt wird. Dabei wurde eine VR Anlage mit Motion Controllern verwendet (Oculus quest 2), damit der Spieler mit virtuellen Händen nach chirurgischen Werkzeugen greifen und diese benutzen kann.

Für gewöhnlich werden die virtuellen Hände des Spielers stets auf die selbe Konfiguration gesetzt, wie die Motion Controller . Dadurch befinden sich die virtuellen Hände immer genau da, wo sich die echten Hände des Spieler befinden, weil dieser die Motion Controller in den Händen hält (siehe Abbildung XXX). 

Um die Immersion des Simulators zu erhöhen, wird ein Kräftesystem entwickelt, das
