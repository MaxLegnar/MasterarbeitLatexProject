...
Mit der Veröffentlichung des Werks \textit{Unified Particle Dynamics} \cite{UPP} lieferte NVIDIA eine umfangreiche Physik Engine namens NVIDIA FleX.  FleX ist frei verfügbar und arbeitet mit den in \cite{UPP} beschriebenen Techniken um PBD-Simulationen besonders effizient auf Grafikkarten zu bringen.
Zuerst lief FleX 1.0 nur auf NVIDIA Grafikkarten, weil die Bibliothek CUDA als Voraussetzung benötigte. Seit NVIDIA FleX 1.2 kann die Software alternativ mit DirectX 11 oder DirectX 12 ausgeführt werden, weswegen nun auch andere Grafikkarten FleX nutzen können (siehe \cite{FlexD3D}).
...
Dieses Problem wird mithilfe eines Prinzips namens \textit{Constraint Averaging} gelöst \cite{UPP}. Dabei werden zuerst alle Constraints parallel verarbeitet und die Korrekturpositionen, $\Delta \textbf{p}_i$, akkumuliert. Nach einer Iteration wird die finale Korrekturposition, $\Delta \widetilde{\textbf{p}}_i$, mit Gleichung \ref{form_LocRel} berechnet \cite{UPP}.

\begin{equation}
\Delta \widetilde{\textbf{p}}_i = \frac{1}{n_i} \Delta \textbf{p}_i
\label{form_LocRel}
\end{equation}

-Mit $n_i$ = Anzahl der Constraints, die auf Partikel $i$ wirken.
