...
Des weiteren implementiert NVIDIA FleX bereits einige bewährte Optimierungstechniken. Beispielsweise wird ein BVH Tree verwendet, um die Partikel-Polygonmodell Kollisionserkennung zu beschleunigen \cite{FlexD3D}. Außerdem wird die Partikelnachbar-Suche und Kollisionserkennung mithilfe von räumlichen Datenstrukturen beschleunigt. Weil hierfür ein reguläres 3D Gitter verwendet wird, müssen alle Partikel aus einer Simulationsszene den selben Radius besitzen \cite{UPP}. FleX Simulationen sind also auf einen konstanten Partikelradius limitiert, dafür bietet NVIDIA FleX eine effiziente Partikel-Kollisionserkenung.
...
%single particle radius sorgt für effizentere kollisionserkennen, weil das 3D Grid auf die größe der Partikel angepasst wird.
...
Für Chirurgische Simulatoren ist vor allem die Soft Body Simulation für virtuelles Gewebe interessant. Für PBD-basierte Simulationen hat sich hierfür das sogenannte \textit{Shape Matching} \cite{Shape} bewährt. NVIDIA FleX setzt Shape Matching für Rigid- als auch für Soft Body Simulationen ein.

Das Grundprinzip des Shape Matchings wird in Abbildung \ref{ShapeMatching} visualisiert. 
...
Die Entspannungskonfiguration einer Partikelgruppe wird gespeichert (siehe Abbildung \ref{ShapeMatching}: \textit{Rest Configuration}). Diese zusammengehörende Partikelgruppe ist dann ein sogenanntes \textit{Cluster}. Die Partikel eines Clusters können nicht miteinander kollidieren. 
