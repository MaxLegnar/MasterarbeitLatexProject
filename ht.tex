...
Die Berechnung von $\Delta \omega$ erfolgt wie zuvor, nur im rotatorischem Sinne (Gleichung \ref{form_omega}):
...
Nun muss berechnet werden, in welche vektorielle Richtung, $\Delta \vec{v_i}$ sich ein Partikel $i$ drehen muss, damit es sich mit der gewünschten Winkelgeschwindigkeit, $\Delta \omega_i$, um die Drehmomentachse dreht. Dabei habe das Partikel die Position $\vec{p_i}$. Abbildung \ref{moment} visualisiert diese Problemstellung.
...
$\Delta \vec{v_i}$ besteht aus einer translatorischen Geschwindigkeit, $s$ und einer Bewegungsrichtung, $\vec{d}$, mit $|\vec{d}|=1$. 
$s$ ist die Umfangsgeschwindigkeit des Partikels, um die gegebene Achse und berechnet sich mit Gleichung \ref{form_speed}.
...
Mit $\vec{u}=\vec{p_i}-\vec{c}$. 

Der Richtungsvektor, $\vec{d}$, muss orthogonal zu $\vec{u}$ und $\vec{d}$ sein und eine Länge von 1 haben. Also kann $\vec{d}$ nach Gleich \ref{form_dir} berechnet werden:

\begin{equation}
\vec{d} = e(\vec{u} \times \vec{d} )
\label{form_dir}
\end{equation}

Wobei mit $e(...)$ der Einheitsvektor aus dem Kreuzprodukt $\vec{u} \times \vec{d} $ gemeint ist.

Schlussendlich ergibt sich die Geschwindigkeit, die zur Partikelgeschiwindigkeit hinzu addiert werden muss durch $\Delta \vec{v_i} = s \cdot \vec{d}$.
