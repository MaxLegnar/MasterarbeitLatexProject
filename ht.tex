...
\chapter{NVIDIA FleX und Unreal Engine 4 für Serius Games}
...
Wenn eine fehlende Funktionalität entdeckt wird, die für die Entwicklung von Serius Games unverzichtbar ist, wird direkt präsentiert, wie die fehlende Funktionalität implementiert wurde.
...
\subsubsection{Über Unreal Engine 4 mit Flex integration}
...
\bild{PerfStats}{12cm}{Alle gemessenen Arbeitsschritte und ihre Bedeutungen \cite{UE4FlexDoc}}
...
%Auch wenn die Messungen viel versprechen, wird sich erst am Ende dieser Arbeit zeigen (siehe Kapitel \ref{chap_Szenarien}), ob NVIDIA Flex für Serius Games im chirurgischen Bereich geeignet ist.
...
Funktioniert schon ganz gut, wobei 3D-modelle nicht mit soft bodies mit rotieren...
...
In der Unreal Integration können Flex-Objekte zwar grundsätzlich miteinander kollidieren, allerdings gibt es nicht die Möglichkeit, auf das Kollisions-Event eines Flex-Objekts zu hören oder eine Kollision zu erkennen.

%Um diese fehlende Funktionalität zu ergänzen, soll das bereits bestehende Kollisionserkennungs-Framework der Unreal Engine 4 genutzt und erweitert werden. Dieses funktioniert nämlich bereits gut und ist mittels Spacial-Data-Ansatz (REFERENZ!!!) optimiert und entsprechend effizient.
...
TODO: Erklären wie gemacht mit einer Bilder-Kollage!
...
Die Unreal Engine 4 besitzt bereits eine traditionelle Physics Engine für Rigid Bodys, die auf NVIDIA PhysX basiert \cite{ue4physics}. Mithilfe der Unreal Engine mit Flex Integration können also sowohl FleX-Simulationen, als auch PhysX-Simulationen ausgeführt werden. Nun stellt sich die Frage ob und wie FleX-Objekte mit PhysX-Objekte interagieren.

\subsection{Kräfte und Momente}

Flex bietet nur RadialForceComponent. Kräfte lassen sich nicht auf einzelne Objekte anwenden

Wofür? Künstliches Greifen, Flüssigkeiten absaugen (naja, das würde wohl einfacher vie partikel-zerstörung gehen
